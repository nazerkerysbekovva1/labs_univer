...
namespace func_lab3
{
    class Program
    {
        static void Main()
        {
            ...
            sections
                .Where(y => section == y.sport_section)
                .ToList()
                .ForEach(x => x.Shygaru(x));
            ...
            Console.WriteLine("\n Orta ball:");
            sportsmens.ForEach(s => Console.WriteLine(Competition.ObshaiaSummaBall(competitions, s)));

            Console.WriteLine("\n Level of skill gained from result of championships:");
            sportsmens.ForEach(s => Console.WriteLine(Competition.ObshaiaSummaBall(competitions, s) + Competition.Skill_level(competitions, s)));
            
            Console.WriteLine("\n Premium:");
            sections.ForEach(s => Console.WriteLine("\n_Секция: " + s.sport_section + "\n_Coach: " + s.coach + "\n_Premium: " + Sport_section.Premium(competitions, sections)));

            Console.WriteLine("#####################################################");
            Console.WriteLine("ABOUT OTHERS!!!");
            ...
            Console.ReadKey();
            Competition.SubstringBefore("Чемпионат2", "Чем");
        }
    }
}