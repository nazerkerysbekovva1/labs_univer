...
namespace func_lab6
{
    class Program
    {
        static void Main()
        {
            ...
            Console.WriteLine();
            var optionalAges = Compe.Map(p => p.id_section).ToArray().ForEach(n => Console.Write(n + " "));
            // => [Some(33), None, Some(37)]

            Console.WriteLine();
            var statedAges = Compe.Bind(p => p.id_section).ToArray();
            ...
            Console.WriteLine("naz");
            Console.ReadKey();
        }
        static IEnumerable<Competition> Compe => new[]
      {
