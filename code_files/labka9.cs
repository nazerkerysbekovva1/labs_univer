...
namespace labka_7
{
    ...
    class Qarsylasu
    {
        ...
        public void Garsylastar(List<OiynTop> topJarys)
        {
            ...
            OiynTop.tsk1 = Task.Factory.StartNew(() =>
            {
                ...
                while (topJarys.Count < OiynTop.top.Count)
                {
                    for (int k = 0; k < OiynTop.top.Count; k++)
                    {
                        if (cancelTok.IsCancellationRequested)
                        {
                            ...
                            Console.WriteLine(" rnd san = " + rand);
                            ...
                            {
                                Console.WriteLine("Рандомно аныктауды токтатуга тусти");
                                cancelTok.ThrowIfCancellationRequested();
                            }
                        }
                    }
        ...
        public void ToptarJarysy(List<OiynTop> topJarys)
        {
            ...
            {
                //cancelTok.ThrowIfCancellationRequested();
                //Console.WriteLine("Jarys() ойын иске косылды");

                for (int i = 0; i < topJarys.Count; i++)
                {
                    //if (cancelTok.IsCancellationRequested)
                    //{
                    //    Console.WriteLine("Ойынды токтатуга сураныс тусти");
                    //    cancelTok.ThrowIfCancellationRequested();
                    //}
                    //Thread.Sleep(500);
                    if (i % 2 != 0)
                ...
                //Console.WriteLine("Jarys() ойын аякталды");
            }); //
        }
    }
    class Program
    {
        static void Main(string[] args)
        {
            ...
            while (counter < n)
            {
                ...
                {
                    if (OiynTop.tsk1.IsCanceled)
                        Console.WriteLine("\ntsk тапсырмасынын орындалуынан бас тарту\n");
                    Console.WriteLine(exc);
                }
                finally
                {
                    OiynTop.tsk1.Dispose();
