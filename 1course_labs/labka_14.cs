...
namespace labka_14
{
    ...
    class Qarsylasu
    {
        ...
        public int Garsylastar(List<OiynTop> topJarys, List<OiynTop> top, int rand)
        {
            ...
            {
                rand = rnd.Next(0, OiynTop.top.Count);
        ...
        public void ToptarJarysy(List<OiynTop> topJarys, List<OiynTop> top)
        {
            Parallel.For(0, topJarys.Count, i =>
            {
                if (i % 2 != 0)
                {
                    Console.WriteLine($"oinaidy>>>  {topJarys[i].Jarys(topJarys, top, (i - 1), i)}");
                }
            });
        }
    }
    class Program
    {
        static void Main(string[] args)
        {
            ...
            while (counter < n)
            {
                ...
                {
                    Console.WriteLine($" Randomno anyktaldy  -  id: {(qq.Garsylastar(topJarys, OiynTop.top, rand)) + 1}");
                }
                qq.ToptarJarysy(topJarys, OiynTop.top);
